...
WIP C# WinForms app for real-time calligraphy & 3D printing via Mitsubishi RV-8CRL-D & CR800. Streams live coordinates over TCP (no pre-programmed waypoints).
...
**3D Printing Demo**
<video src="Robot 3D Printing UI.mp4" controls="controls" style="max-width: 100%;"></video>
...
- **Pipelines**:
  ...
  - `PrusaSlicerStrategy` (3D): Headless PrusaSlicer CLI execution.
  - `GCodeParser` (3D): Parses G-Code to 6DOF waypoints (X,Y,Z, Posture).
- **Networking (`RobotTcpClient`)**: Decoupled. Handles bidirectional TCP handshakes & coordinate streaming (`MVS; X; Y; Z`).

---

## ✨ Key Features

- **🧊 3D Printing**: Headless slicing (`prusa-slicer-console.exe`), auto-centering, interactive 3D orbit viewer, layer debugger.
...
- **⚡ Control**: Zero-latency streaming, Continuous Interpolation (`CNT 1`), instant Pause/Stop commands.
...
- **PrusaSlicer**: Ensure `prusa-slicer-console.exe` is in PATH or working dir.
...
5. **Load PRG**: Write `RobotListener_SingleLoop.prg` to controller.

---

## 🧪 Testing

1. Run `RobotListener_SingleLoop.prg` on robot.
